...
class CountingValue
{
  public:
    ...
    CountingValue(const CountingValue &other)
        : m_copyConstructCount(++other.m_copyConstructCount), m_moveConstructCount(other.m_moveConstructCount),
          m_copyAssignCount(other.m_copyAssignCount), m_moveAssignCount(other.m_moveAssignCount),
    ...
    bool checkCounts(unsigned int copyConstructCount, unsigned int moveConstructCount, unsigned int copyAssignCount,
                     unsigned int moveAssignCount, bool movedFrom) const
    {
        return m_copyConstructCount == copyConstructCount && m_moveConstructCount == moveConstructCount &&
               m_copyAssignCount == copyAssignCount && m_moveAssignCount == moveAssignCount && m_movedFrom == movedFrom;
    }
...
{
  public:
    Checker(CountingLock &lock) : m_lock(lock)
    {
    }
    Checker(const Checker &other)
        : m_isOk(other.m_isOk && other.m_lock.isLocked() && !other.m_lock.isFaulted()), m_lock(other.m_lock),
    ...
    bool checkValueCounts(bool copyConstructed, bool moveConstructed, unsigned int copyAssignCount,
                          unsigned int moveAssignCount, bool movedFrom) const
    {
...
TEST_CASE("Copy member function locks the mutex before copying (and unlocks afterwards!)")
{
    ...
    safe::Safe<Checker &, CountingLock &> safeChecker(checker, lock);
